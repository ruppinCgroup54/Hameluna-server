...
namespace hameluna_server.Controllers
{
    ...
    public class AdopterController : Controller
    {
        ...
        [HttpGet]
            ...
            public IActionResult Post([FromBody] JsonElement ad)
            {
                try
                {

                    JsonElement addr = ad.GetProperty("address");
                    JsonElement adop = ad.GetProperty("adopter");


                    Address? address =  addr.Deserialize<Address>();
                    Adopter? adopter = adop.Deserialize<Adopter>();

                    int newAddress = address.Insert();

                     adopter.Address.Id = newAddress;

                    string newId = adopter.Insert();
